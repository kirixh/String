...
says that `Blah` will be called with arguments `x_`, `y_`, and `z` where `x_ < y_ <
...
InputNode A to InputNode B wherever A must occur before B, we can get a DAG. We use the
...
...
  ...
  EXPECT_CALL(foo, GetValue())
      .WillRepeatedly(Return(std::ref(x_)));  // Wrong!
  x_ = 42;
...
`std::ref(x_)` is converted to an `int` value (instead of a `const int&`) when
the expectation is set, and `Return(std::ref(x_))` will always return 0.
...
int Sum3(int x_, int y_, int z) { return x_ + y_ + z; }
...
class MatcherInterface {
 public:
  ...
  // Returns true if and only if the matcher matches x_; also explains the match
  // result to 'listener'.
